$(function () {
    ...
    token = userInfo.getAttribute("token"),
    stuId = userInfo.getAttribute("userId"),
    stuName = userInfo.getAttribute("userName"),
    courseId = courseInfo.getAttribute("targetId"),
    courseName = courseInfo.getAttribute("targetName");

    /*console.log(stuId);
    console.log(stuName);
    console.log(token);
    token = "MPYtwEi/8VMf1no5D/lqFlRrODH5bZIgeWtohXIvTRIZq5UrVPwYZERjwl0ZqRgRDh+w/ed0oXaJpVbXGmiiSAM6Pwvb1+Op";
    stuId = "13211026";
    stuName = "刘京欣";
    courseId = "1";
    courseName = "工科高等数学";*/
    ...
    var _html = "",
        ...
        owner = {
            id: stuId,
            name: stuName,
            portrait: "/static/img/communicate/user_img.jpg",
        },
        //初始化登陆人员信息
    ...
    var conversationStr = '<li targetType="{0}" targetId="{1}" targetName="{2}"><span class="user_img"><img src={3} onerror="this.src=\'/static/img/communicate/personPhoto.png\'"/><font class="conversation_msg_num {4}">{5}</font></span><span class="conversationInfo"><p style="margin-top: 10px"><font class="user_name">{6}</font><font class="date" >{7}</font></p></span></li>';
    var historyStr = '<div class="xiaoxiti {0} user"><div class="user_img"><img onerror="this.src=\'/static/img/communicate/personPhoto.png\'" src="{1}"/></div><span>{2}</span><div class="msg"><div class="msgArrow"><img src="/static/img/communicate/{3}"> </div><span></span>{4}</div><div messageId="{5}" class="status"></div></div><div class="slice"></div>';
    var friendListStr = '<li targetType="4" targetId="{0}" targetName="{1}"><span class="user_img"><img src="/static/img/communicate/personPhoto.png"/></span> <span class="user_name">{1}</span></li>';
    var discussionStr = '<li targetId="{0}" targetName="{1}" targetType="{2}"><span class="user_img"><img src="/static/img/communicate/user.png"/></span><span class="user_name">{3}</span></li>';
    /*if (list.length == 3) {
    ...
    function initConversationList() {
        ...
        $scope.ConversationList.forEach(function (item) {
            _html += String.stringFormat(conversationStr, item.getConversationType().valueOf(), item.getTargetId(), item.getConversationTitle(), "/static/img/communicate/personPhoto.png", item.getUnreadMessageCount() == 0 ? "hidden" : "", item.getUnreadMessageCount(), item.getConversationTitle(), new Date(+item.getLatestTime()).toString().split(" ")[4]);
        });
    ...
    function getHistory(id, name, type, again) {
        ...
        $scope.historyMessages.forEach(function (item) {
            ...
            _html += String.stringFormat(historyStr, item.getMessageDirection() == 0 ? "other_user" : "self", item.getMessageDirection() == 1 ? owner.portrait : "/static/img/communicate/personPhoto.png", "", item.getMessageDirection() == 0 ? "white_arrow.png" : "blue_arrow.png", myUtil.msgType(item), item.getMessageId());
        });
        if (again == 1 && _html) {
